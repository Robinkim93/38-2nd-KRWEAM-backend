...
const signUp = async (nickName, phoneNumber, email, password) => {
	...
	return await userDao.createUserWithEmail(nickName, phoneNumber, email, hashedPassword)
}
...
const kakaoLogIn = async(userInfo) => {
    ...
    console.log(user);
    if(user.length==0){
        userId = await userDao.createUserWithKakao(userInfo.properties.nickname, userInfo.id);
    } else {
        userId = user[0].id;
    }
